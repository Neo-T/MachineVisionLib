...
{
	FaceDatabase *pobjFaceDB = (FaceDatabase*)dw64InputParam;

	pobjFaceDB->o_pobjVideoPredict->Predict(mVideoData);
...
{	
	...
	//pstParam->pobjCCF->detectMultiScale(mGray, vFaces, 1.1, 3, 0, Size(50, 50), Size(500, 500));
	...
	//for (INT i = 0; i < vFaces.size(); i++)
	{
		...
		//INT nCurLTX = static_cast<INT>(vFaces[i].tl().x) - 40;
		//INT nCurLTY = static_cast<INT>(vFaces[i].tl().y) - 30;
		//INT nCurRBX = static_cast<INT>(vFaces[i].br().x) + 40;
		//INT nCurRBY = static_cast<INT>(vFaces[i].br().y) + 30;
		...
		Face objFace(0, 0, nCurRBX - nCurLTX, nCurRBY - nCurLTY);
		string strPersonName;		
