...
//* GAN将图像清晰
//* 文章：https://baijiahao.baidu.com/s?id=1596169253423562530&wfr=spider&for=pc
...
void OpenImgeFile(CHAR *pszImgFileName)
{
	...
	//* 使用多字节转换函数mbstowcs()是注意要在ImagMagic属性中实现声明预处理宏_CRT_SECURE_NO_WARNINGS，声明方式如下：
	//* ImagMagic属性->C/C++->预处理器->预处理器定义，将_CRT_SECURE_NO_WARNINGS添加到后面即可
	if (mbstowcs(wbImgFileName, pszImgFileName, MAX_PATH) < 0)
	{
		MessageBox(hMainWnd, "mbstowcs()函数调用出错，请联系尼奥，他是墨菲斯相信能拯救锡安的人！", "错误", MB_OK);
		return;
	}
